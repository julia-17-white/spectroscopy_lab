...
def peak_finding(df, peak_width):
...
def delta_t_fit(df, peaks):
    plt.figure()
    y_ax = np.arange(len(df['x-axis'][peaks[0][2:-1]]))
    # plt.scatter(df['x-axis'][peaks[0][2:-1]], peaks[1]['prominences'][2:-1] + 0.36, s=15)
    plt.scatter(df['x-axis'][peaks[0][2:-1]], y_ax)

    lin_fit = lin_fitting(df['x-axis'][peaks[0][2:-1]], y_ax)
    plt.plot(df['x-axis'][peaks[0][2:-1]], lin_fit[0], color='darkgreen', label = 'linear fit')
    
    x_data, y_data = pol_fitting(df['x-axis'][peaks[0][2:-1]], y_ax)
...
def main():
    data_df = obtain_data()
    peaks = peak_finding(data_df, 10)
    time_diff1 = delta_t_calc(data_df, peaks)
    time_diff2 = delta_t_fit(data_df, peaks)
